...
class ofxBoidFx : public ofxFXObject {
 public:
  ...
  ofxBoidFx(){
    ...
    fragmentShader = STRINGIFY(uniform vec2 resolution;
 ...
 public:
  ...
  ofxGravurFX(){
    ...
    fragmentShader = STRINGIFY(
				 ...
				 if (mask == vec4(0.0,0.0,0.0,1.0)) {
				   ...
				   image.xyzw = vec4(1.0,1.0,1.0,1.0);
				   //image = mask;
				 } else{
				   if (time == 1) {
				     //image.w = min(1.0,image.w + 1.00);// <<< ici tim 0.0038
				     // image.rgba = vec4(1.0,1.0,1.0,0.0);
				     image.xyzw = vec4(1.0, 1.0, 1.0, i);
				   }
				 }
				 gl_FragColor = image;
			       }
  ...
  void update(){
	...
	shader.setUniform2f(texRes.c_str() , (float)textures[i].getWidth(), (float)textures[i].getHeight());
      ...
      shader.setUniform1f("i",(float) _i);
      injectUniforms();
